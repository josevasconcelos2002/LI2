...
int sepString(STACK *s,char* token) {
    int r = 0;
    if((strcmp(token,"/") == 0) && (s->stack[s->sp].type == STR) && (s->stack[s->sp - 1].type == STR )) {
        ...
        aux.type = STR;
        arr.value.arrayValue = new_stack();
        char *lim = strtok(str.value.stringValue,subStr.value.stringValue);
        while(lim) {
            aux.value.stringValue = strdup(lim);
            push(arr.value.arrayValue,aux);
            lim = strtok(NULL,subStr.value.stringValue);
        }
        push(s,arr);
        r = 1;
