...
/** 
 * 
 * @A Função que divide a linha em tokens para o tratamento dos mesmo
 * 
 * @É line .
 * 
...
int handle(STACK *s, DATA* values,  char *token){
	 ...
	 || byRange(s,token)) || getElem(s,token) || searchSubString(s,token) || removeElem(s,token) || sepString(s,token) ;
